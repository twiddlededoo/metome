...
export const uploadFileToSession = createServerFn({ method: 'POST' })
  .inputValidator((data: { sessionId: string; fileName: string; fileType: string; fileSize: number; fileData: string }) => data)
  .handler(async ({ data }) => {
    const supabase = getSupabase();
    const { data: session, error } = await supabase
      .from('upload_sessions')
...
export const getUploadedFileData = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (error || !session || !session.file_data) return null;
